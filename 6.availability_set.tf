...
resource "azurerm_network_interface" "app-grp-network-interface" {
  ...
  ip_configuration {
    name                          = "internal"
    subnet_id                     = azurerm_subnet.app-grp-subnet-A.id
...
resource "azurerm_windows_virtual_machine" "app-grp-vm" {
  ...
  availability_set_id = azurerm_availability_set.app-grp-availability_set.id
...
resource "azurerm_availability_set" "app-grp-availability_set" {
    name = "app-grp-availability_set"
    location = local.location
    resource_group_name = local.resource_group
